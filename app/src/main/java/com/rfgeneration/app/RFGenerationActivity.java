...
import com.rfgeneration.app.service.RFGenerationService;
...
public class RFGenerationActivity extends AppCompatActivity implements OnClickListener, LoaderManager.LoaderCallbacks<Cursor> {
    ...
    public TableRow createFolderRow(Folder folder, int index, int depth, boolean selected) {
    	...
    	if(folder.getTimestamp() > 0) {
    		...
    		folderNameText.setTextColor(getResources().getColor(R.color.text));
    		...
    		folderSyncText.setTextColor(getResources().getColor(R.color.text));
    	...
    	if(selected) {
    		...
    		folderRow.setBackgroundResource(R.color.selected);
    ...
    public void selectFolder(int index) {
    	...
    	folderRow.setBackgroundResource(R.color.body);
